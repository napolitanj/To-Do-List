...
function userInterface() {
  ...
  function listItems(project) {
    project.items.forEach((item) =>
      itemList.appendChild(renderItem(project, item), sP())
    );
    sP();
  }
  ...
  function updateToday() {
    projects.forEach((e) => checkForToday(e));
    function checkForToday(project) {
      project.items.forEach((item) => {
        if (
          isToday(new Date(item.date)) === true &&
          today.items.indexOf(item) === -1
        ) {
  ...
  // Load Projects
  function lP() {
    const projectCollection = JSON.parse(localStorage.getItem("storedProjects"));
